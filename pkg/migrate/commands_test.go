...
import (
	...
	"gitlab.com/laboct2021/pnl-game/pkg/migrate"
)
...
func (s *CreateCmdSuite) mustWriteFile(dir, file, body string) {
	if err := ioutil.WriteFile(filepath.Join(dir, file), []byte(body), 0644); err != nil { //nolint:gosec //need to edit
		s.FailNow(err.Error())
	}
}
...
func (s *CreateCmdSuite) TestNextSeqVersion() {
	cases := []struct {
		...
		{"Bad digits", []string{}, 0, "", migrate.ErrInvalidSequenceWidth},
	...
	for _, c := range cases {
		s.Run(c.tid, func() {
			v, err := migrate.NextSeqVersion(c.matches, c.seqDigits)
...
var timeDate = time.Date(2000, 12, 25, 00, 01, 02, 3456789, time.UTC) //nolint:gocritic //need to edit
...
func (s *CreateCmdSuite) TestTimeVersion() {
	...
	cases := []struct {
		...
		{"Bad format", ts, "", "", migrate.ErrInvalidTimeFormat},
	...
	for _, c := range cases {
		s.Run(c.tid, func() {
			v, err := migrate.TimeVersion(c.time, c.format)
...
func (s *CreateCmdSuite) TestCreateCmd() {
	...
	cases := []struct {
		...
		{"seq and format", nil, "", nil, nil, migrate.ErrIncompatibleSeqAndFormat, ".", ts, "unix", true, 4, "sql", "name"},
		{"seq init dir dot", nil, "", nil, []string{"0001_name.up.sql", "0001_name.down.sql"}, nil, ".", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq init dir dot trailing slash", nil, "", nil, []string{"0001_name.up.sql", "0001_name.down.sql"}, nil, "./", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq init dir double dot", []string{"subdir"}, "subdir", nil, []string{"0001_name.up.sql", "0001_name.down.sql"}, nil, "..", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq init dir double dot trailing slash", []string{"subdir"}, "subdir", nil, []string{"0001_name.up.sql", "0001_name.down.sql"}, nil, "../", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq init dir absolute", []string{"subdir"}, "", nil, []string{"subdir/0001_name.up.sql", "subdir/0001_name.down.sql"}, nil, "/subdir", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq init dir absolute trailing slash", []string{"subdir"}, "", nil, []string{"subdir/0001_name.up.sql", "subdir/0001_name.down.sql"}, nil, "/subdir/", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq init dir relative", []string{"subdir"}, "", nil, []string{"subdir/0001_name.up.sql", "subdir/0001_name.down.sql"}, nil, "subdir", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq init dir relative trailing slash", []string{"subdir"}, "", nil, []string{"subdir/0001_name.up.sql", "subdir/0001_name.down.sql"}, nil, "subdir/", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq init dir dot relative", []string{"subdir"}, "", nil, []string{"subdir/0001_name.up.sql", "subdir/0001_name.down.sql"}, nil, "./subdir", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq init dir dot relative trailing slash", []string{"subdir"}, "", nil, []string{"subdir/0001_name.up.sql", "subdir/0001_name.down.sql"}, nil, "./subdir/", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq init dir double dot relative", []string{"subdir"}, "subdir", nil, []string{"subdir/0001_name.up.sql", "subdir/0001_name.down.sql"}, nil, "../subdir", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq init dir double dot relative trailing slash", []string{"subdir"}, "subdir", nil, []string{"subdir/0001_name.up.sql", "subdir/0001_name.down.sql"}, nil, "../subdir/", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq init dir maze", []string{"subdir"}, "subdir", nil, []string{"0001_name.up.sql", "0001_name.down.sql"}, nil, "..//subdir/./.././/subdir/..", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq width invalid", nil, "", nil, nil, migrate.ErrInvalidSequenceWidth, ".", ts, migrate.DefaultTimeFormat, true, 0, "sql", "name"},
		{"seq malformed", nil, "", []string{"bad.sql"}, []string{"bad.sql"}, errors.New("malformed migration filename: bad.sql"), ".", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq not int", nil, "", []string{"bad_bad.sql"}, []string{"bad_bad.sql"}, errors.New(`strconv.ParseUint: parsing "bad": invalid syntax`), ".", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq negative", nil, "", []string{"-5_negative.sql"}, []string{"-5_negative.sql"}, errors.New(`strconv.ParseUint: parsing "-5": invalid syntax`), ".", ts, migrate.DefaultTimeFormat, true, 4, "sql", "name"},
		{"seq increment", nil, "", []string{"3_three.sql", "4_four.sql"}, []string{"3_three.sql", "4_four.sql", "0005_five.up.sql", "0005_five.down.sql"}, nil, ".", ts, migrate.DefaultTimeFormat, true, 4, "sql", "five"},
		{"seq overflow", nil, "", []string{"9_nine.sql"}, []string{"9_nine.sql"}, errors.New(`next sequence number 10 too large. At most 1 digits are allowed`), ".", ts, migrate.DefaultTimeFormat, true, 1, "sql", "ten"},
		{"time empty format", nil, "", nil, nil, migrate.ErrInvalidTimeFormat, ".", ts, "", false, 0, "sql", "name"},
	...
	for i := range cases {
		...
		s.Run(c.tid, func() {
			...
			err := migrate.CreateCmd(dir, c.startTime, c.format, c.name, c.ext, c.seq, c.seqDigits, false)
...
func TestNumDownFromArgs(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			num, needsConfirm, err := migrate.NumDownMigrationsFromArgs(c.applyAll, c.args)
