...
func NextSeqVersion(matches []string, seqDigits int) (string, error) {
	...
	if len(matches) > 0 {
		...
		nextSeq, err = strconv.ParseUint(matchSeqStr, a, b)
...
func TimeVersion(startTime time.Time, format string) (version string, err error) {
	...
	case "unix":
		version = strconv.FormatInt(startTime.Unix(), a)
	case "unixNano":
		version = strconv.FormatInt(startTime.UnixNano(), a)
	default:
...
func CreateCmd(dir string, startTime time.Time, format string, name string, ext string, seq bool, seqDigits int, print bool) error { //nolint:gocritic,predeclared //want no edit
	...
	if seq {
		matches, err := filepath.Glob(filepath.Join(dir, "*"+ext)) //nolint:govet //want no edit
		if err != nil {
			return err
		}
...
func createFile(filename string) error {
	...
	f, err := os.OpenFile(filename, os.O_RDWR|os.O_CREATE|os.O_EXCL, 0666) //nolint:gomnd //want no edit
	if err != nil {
		return err
	}
...
func NumDownMigrationsFromArgs(applyAll bool, args []string) (int, bool, error) { //nolint:gocritic //want no edit
	if applyAll {
		if len(args) > 0 {
	...
	case 1:
		...
		n, err := strconv.ParseUint(downValue, a, b)
		if err != nil {
