...
import (
	...
	_ "github.com/golang-migrate/migrate/v4/database/postgres" //nolint:nolintlint //want no edit
...
func newFlagSetWithHelp(name string) (*flag.FlagSet, *bool) { //nolint:gocritic //want no edit
...
func Main(version string) { //nolint:gocyclo //want no edit
	helpPtr := flag.Bool("help", false, "")
	versionPtr := flag.Bool("version", false, "")
	verbosePtr := flag.Bool("verbose", false, "")
	prefetchPtr := flag.Uint("prefetch", 10, "")        //nolint:nolintlint,goimports,gomnd //want no edit
	lockTimeoutPtr := flag.Uint("lock-timeout", 15, "") //nolint:gomnd //want no edit
	pathPtr := flag.String("path", "", "")
	databasePtr := flag.String("database", "postgres://user:mypassword@localhost:5432/pnl_game?sslmode=disable", "")
	sourcePtr := flag.String("source", "file://db/migration", "")
...
Database drivers: `+strings.Join(database.List(), ", ")+"\n", CreateUsage, gotoUsage, upUsage, downUsage, dropUsage, forceUsage)
	...
	dbs := db.Setup(config.LoadConfig("./config/config.yaml").DB)
	*databasePtr = dbs.String()
	...
	case "goto":
		...
		v, err := strconv.ParseUint(gotoSet.Arg(0), a, b)
		if err != nil {
	...
	case "up":
		...
		if upSet.NArg() > 0 {
			n, err := strconv.ParseUint(upSet.Arg(0), a, b)
			if err != nil {
	...
	case "force":
		...
		v, err := strconv.ParseInt(forceSet.Arg(0), a, b)
		if err != nil {
			log.fatal("error: can't read version argument V")
		}
